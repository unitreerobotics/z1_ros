 ...
 */
bool NearZero(const double& val, double threhold = 0.000001);

Eigen::VectorXd rpyxyz_to_xyzrpy(const Eigen::VectorXd& rpyxyz);
Eigen::VectorXd xyzrpy_to_rpyxyz(const Eigen::VectorXd& xyzrpy);
...
/*
 * Function: Build Homogeneous Matrix by x axis, y axis and p 
 */
Mat4 homoMatrix(Vec3 x, Vec3 y, Vec3 p);

Mat4 homoMatrix(Vec3 linear, Vec3 angular);

/**
 * @brief Determines whether the given matrix represents a valid homogeneous transformation matrix.
 */
bool isHomoMat(const Mat4& T);
